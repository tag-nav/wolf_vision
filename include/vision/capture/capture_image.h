...
class WKeyPoint
{
  ...
  public:
    ...
    cv::KeyPoint getCvKeyPoint() const
    {
        return cv_kp_;
    }
    void setCvKeyPoint(cv::KeyPoint _cv_kp)
    {
        cv_kp_ = _cv_kp;
    }

    Eigen::Vector2d getEigenKeyPoint() const
    {
        return Eigen::Vector2d(cv_kp_.pt.x, cv_kp_.pt.y);
    }
    cv::Point2d getCvPoint() const
    {
        return cv_kp_.pt;
    }
...
class CaptureImage : public CaptureBase
{
  ...
  public:
    ...
    const KeyPointsMap getKeyPoints() const
    {
        return mwkps_;
    }
    void setKeyPoints(const KeyPointsMap& _mwkps)
    {
        mwkps_ = _mwkps;
    }

    const TracksMap& getTracksPrev() const
    {
        return tracks_prev_;
    }
    void setTracksPrev(const TracksMap& _tracks)
    {
        tracks_prev_ = _tracks;
    }

    const TracksMap& getTracksOrigin() const
    {
        return tracks_origin_;
    }
    void setTracksOrigin(const TracksMap& _tracks)
    {
        tracks_origin_ = _tracks;
    }

    bool getLastWasRepopulated() const
    {
        return last_was_repopulated_;
    }
    void setLastWasRepopulated(bool _repopulated)
    {
