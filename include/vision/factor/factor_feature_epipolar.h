...
class FactorFeatureEpipolar : public FactorAutodiff<FactorFeatureEpipolar, 1, 3, 4, 3, 4, 3, 4>
{
    public:
                         ...
                         T* _residuals) const
        {
            ...
            Matrix<T, 3, 1> u_own   = getFeature()->getMeasurement().cast<T>();
            Matrix<T, 3, 1> u_other = getFeatureOther()->getMeasurement().cast<T>();
            ...
            // Covariance of error
            Matrix<T, 1, 1> Q_e = J_e_uown   * getFeature()     ->getMeasurementCovariance() * J_e_uown.transpose()
                                + J_e_uother * getFeatureOther()->getMeasurementCovariance() * J_e_uother.transpose();
    ...
    private:
        ...
        Matrix3s K_, K_inv_; ///< Intrinsic matrix and its inverse
