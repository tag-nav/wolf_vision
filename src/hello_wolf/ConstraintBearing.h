...
class ConstraintBearing : public ConstraintAutodiff<ConstraintBearing, 1, 2, 1, 2>
{
    public:
        ...
        template<typename T>
        inline bool ConstraintBearing::operator ()(const T* const _p1,
                                                   const T* const _o1,
                                                   const T* const _p2,
                                                   T* _res) const;
                                                                    ...
                                                                    const T* const _p2, T* _res) const
{
    ...
    Map<const Translation<T, 2>>    position   (_p1);
    Rotation2D<const T>             orientation(*_o1);
    ...
    Transform<T, 2, Affine> H_w_r ( position * orientation ); // Robot frame = robot-to-world transform
    ...
    T bearing   = atan(point_r(1), point_r(0));
