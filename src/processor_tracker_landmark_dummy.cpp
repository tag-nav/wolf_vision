...
{
    std::cout << "finding " << _landmark_list_in.size() << " landmarks..." << std::endl;
...
unsigned int ProcessorTrackerLandmarkDummy::detectNewFeatures(const unsigned int& _max_features)
{
    ...
    for (unsigned int i = 1; i <= _max_features; i++)
    {
        ...
        new_features_last_.push_back(
                new FeatureBase(FEAT_POINT_IMAGE, n_feature_ * Eigen::Vector1s::Ones(), Eigen::MatrixXs::Ones(1, 1)));
...
{
    std::cout << "creating landmark..." << std::endl;
...
{
    std::cout << "creating constraint:" << std::endl;
