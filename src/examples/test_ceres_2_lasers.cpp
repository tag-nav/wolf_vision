...
int main(int argc, char** argv)
{
    ...
    SensorOdom2D* odom_sensor = new SensorOdom2D(new StateBlock(odom_pose.head(2)), new StateBlock(odom_pose.tail(1)), odom_std_factor, odom_std_factor);
    SensorGPSFix* gps_sensor = new SensorGPSFix(new StateBlock(gps_pose.head(2)), new StateBlock(gps_pose.tail(1)), gps_std);
    SensorLaser2D* laser_1_sensor = new SensorLaser2D(new StateBlock(laser_1_pose.head(2)), new StateBlock(laser_1_pose.tail(1)));
    SensorLaser2D* laser_2_sensor = new SensorLaser2D(new StateBlock(laser_2_pose.head(2)), new StateBlock(laser_2_pose.tail(1)));
    laser_1_sensor->setScanParams(laserscanutils::LaserScanParams({M_PI/2,-M_PI/2, -M_PI/720,0.01,0.2,100,0.01,0.01}));
    laser_2_sensor->setScanParams(laserscanutils::LaserScanParams({M_PI/2,-M_PI/2, -M_PI/720,0.01,0.2,100,0.01,0.01}));
    ...
    for (unsigned int step = 1; step < n_execution; step++)
    {
        ...
        t1 = clock();
        robot.render(laser_1_processor->getLastPtr() == nullptr ? FeatureBaseList({}) : *laser_1_processor->getLastPtr()->getFeatureListPtr(), *problem->getMapPtr()->getLandmarkListPtr(), problem->getCurrentState());
...
//		std::cout << "\nTree after step..." << std::endl;
    ...
    // Landmarks
    i = 0;
    Eigen::VectorXs landmarks(problem->getMapPtr()->getLandmarkListPtr()->size() * 2);
