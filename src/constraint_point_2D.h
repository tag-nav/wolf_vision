...
class ConstraintPoint2D : public ConstraintSparse<3, 2, 1, 2, 1>
{
    public:
        ...
        ConstraintPoint2D(FeatureBase* _ftr_ptr, FrameBase* _frame_ptr, ConstraintStatus _status = CTR_ACTIVE) :
                ConstraintSparse<3, 2, 1, 2, 1>(_ftr_ptr, CTR_IMG_PNT_TO_EP, _frame_ptr, _status, _frame_ptr->getPPtr(), _frame_ptr->getOPtr(), _ftr_ptr->getFramePtr()->getPPtr(), _ftr_ptr->getFramePtr()->getOPtr())
        {
            //
        }
