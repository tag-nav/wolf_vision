...
CaptureBasePtr cap1 = frm1->addCapture(std::make_shared<CaptureMotion>(1, nullptr, delta));
...
TEST(ConstraintOdom3D, fix_0_solve)
{
    ...
    ASSERT_EIGEN_APPROX(frm1->getState(), delta);
...
TEST(ConstraintOdom3D, fix_1_solve)
{
    ...
    ASSERT_EIGEN_APPROX(frm0->getState(), problem->zeroState());
}
