                                            ...
                                            const T* const _landmarkO, T* _residuals) const
{
    ...
    inverse_R_robot << cos(*_robotO), sin(*_robotO), -sin(*_robotO), cos(*_robotO);
    // Expected measurement
    Eigen::Matrix<T, 2, 1> expected_landmark_relative_position = inverse_R_sensor
            * (inverse_R_robot * (landmark_position - robot_position) - sensor_position);
    T expected_landmark_relative_orientation = (*_landmarkO) - (*_robotO)
            - atan2(inverse_R_sensor(0, 1), inverse_R_sensor(0, 0));
    ...
    while (expected_landmark_relative_orientation <= T(-M_PI))
        expected_landmark_relative_orientation = expected_landmark_relative_orientation + T(2 * M_PI);
    // Residuals
    _residuals[0] = (expected_landmark_relative_position(0) - T(getMeasurement()(0)))
            / T(sqrt(getMeasurementCovariance()(0, 0)));
    _residuals[1] = (expected_landmark_relative_position(1) - T(getMeasurement()(1)))
            / T(sqrt(getMeasurementCovariance()(1, 1)));
